...
use crate::{
    bold_entry, err, filesystem as wfs, g, qprint, r,
    registry::{
...
pub(crate) struct CpOpts {
    ...
    #[clap(
        ...
        short = 't',
        takes_value = true,
        long_help = "By default, `cp` will copy all tags and values of those tags to the results from the \
                     query"
    )]
    ...
    #[clap(
        ...
        long = "glob",
        takes_value = false,
        long_help = "Use a glob to match the input path instead of matching files in the local directory. \
                     This argument doesn't do anything, and is a placeholder. If the global option is \
                     used, pattern matching is turned on and file-matching is no longer used"
    )]
    pub(crate) glob: bool,
...
impl App {
    ...
    pub(crate) fn cp(&mut self, opts: &CpOpts) -> Result<()> {
        ...
        let path = &fs::canonicalize(&opts.input_path)
            .context(format!("{}: failed to canonicalize", opts.input_path.display()))?;
                    ...
                    match filetags {
                        Ok(filetags) => {
                            ...
                            for ft in filetags.iter() {
                                ...
                                let constructed = FileTag::new(entry.id(), ft.tag_id(), ft.value_id());
                    ...
                    match filetags {
                        Ok(filetags) => {
                            if filetags.is_empty() {
                                wutag_fatal!(
                                    "{}: has no tags. Please clean your registry or give this file tags",
                                    bold_entry!(path)
                                );
                            }
                            ...
                            let mut combos = opts
                                ...
                                .map(|(t, v)| -> Result<TagValueCombo> {
                                    ...
                                    let value = reg.value_by_name(v).map(|value| {
                            ...
                            let mut opt_tag = opts.tag.as_ref().map_or_else(Vec::new, |tag| {
                                if let Ok(found) = reg.tag_by_name(tag) {
                                    if filetags.iter().any(|ft| ft.tag_id() == found.id()) {
                                        ...
                                        wutag_fatal!("{}: does not have tag {}", bold_entry!(path), tag);
                                    }
                                } else {
                                    wutag_fatal!(
                            ...
                            crawler(
                                ...
                                |entry: &ignore::DirEntry| {
                                    ...
                                    let mut dest = reg.file_by_path(path);

                                    if dest.is_err() {
                                        log::debug!("{}: inserting file", path.display());
                                        dest = reg.insert_file(path);
                                    }
                                    ...
                                    if combos.is_empty() {
                                        ...
                                        for combo in &combos {
                                            let constructed =
                                                FileTag::new(dest.id(), combo.tag_id(), combo.value_id());
                                            ...
                                            let to_insert =
                                                FileTag::new(file.id(), combo.tag_id(), combo.value_id());
                                            if let Err(e) = reg.copy_filetag_fileid(&to_insert, dest.id()) {
                                                if combo.value_id().id() == 0 {
                                                    wutag_error!(
                                                        "{}: failed to copy {}",
                                                        bold_entry!(entry),
